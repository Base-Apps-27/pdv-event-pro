...
function buildFormData(segment, suggestedStartTime) {
  return {
    ...
    // FIX 2026-03-06 (v2): Number-type fields must default to null, not "".
    // "" is invalid for schema type "number" and causes 422.
    video_length_sec: segment?.video_length_sec ?? null,
    art_types: segment?.art_types || [],
    drama_handheld_mics: segment?.drama_handheld_mics ?? null,
    drama_headset_mics: segment?.drama_headset_mics ?? null,
    ...
    dance_handheld_mics: segment?.dance_handheld_mics ?? null,
    dance_headset_mics: segment?.dance_headset_mics ?? null,
    ...
    // FIX 2026-03-06: Array-type fields MUST default to [] not "" to prevent 422 schema validation errors.
    // Enum-type fields MUST default to undefined/null (not "") to avoid sending invalid enum values.
    ...
    // Spoken Word fields (2026-02-28: added per arts admin feedback)
    // FIX 2026-03-06: spoken_word_mic_position is enum — use undefined instead of "" to avoid 422
    spoken_word_mic_position: segment?.spoken_word_mic_position || undefined,
    spoken_word_has_music: segment?.spoken_word_has_music || false,
    spoken_word_music_title: segment?.spoken_word_music_title || "",
    spoken_word_music_url: segment?.spoken_word_music_url || [],
    spoken_word_music_owner: segment?.spoken_word_music_owner || "",
    spoken_word_notes: segment?.spoken_word_notes || "",
    spoken_word_description: segment?.spoken_word_description || "",
    spoken_word_speaker: segment?.spoken_word_speaker || "",
    spoken_word_script_url: segment?.spoken_word_script_url || [],
    spoken_word_audio_url: segment?.spoken_word_audio_url || [],
    // Painting fields (2026-02-28: added per arts admin feedback)
    painting_needs_easel: segment?.painting_needs_easel || false,
    painting_needs_drop_cloth: segment?.painting_needs_drop_cloth || false,
    painting_needs_lighting: segment?.painting_needs_lighting || false,
    painting_canvas_size: segment?.painting_canvas_size || "",
    painting_other_setup: segment?.painting_other_setup || "",
    painting_notes: segment?.painting_notes || "",
    ...
    // FIX 2026-03-06: presentation_url and notes_url are array types in schema
