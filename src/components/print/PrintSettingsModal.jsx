...
export default function PrintSettingsModal({ open, onOpenChange, settingsPage1, settingsPage2, onSave, language = "es", serviceData = null }) {
  ...
  // Prioritize custom service if segments exist and have items
  const isCustomService = Array.isArray(serviceData?.segments) && serviceData.segments.length > 0;
  // Fallback to weekly if not custom, but allow empty 9:30am for new weekly services
  ...
  function renderStandardPreview() {
    ...
    return (
      ...
      >
        {/* Logo and Margins - Same as standard */}
          ...
          <div style={{ width: '100%' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', fontSize: `${BASE_BODY * page1Settings.bodyFontScale}px`, lineHeight: 1.4 }}>
              ...
              <div>
                ...
                {/* ... (Standard Service Rendering Logic for 9:30am) ... */}
                {renderStandardSegments(serviceData?.['9:30am'])}
              </div>
              {/* 11:30 AM Column */}
              <div>
                ...
                {/* ... (Standard Service Rendering Logic for 11:30am) ... */}
                {renderStandardSegments(serviceData?.['11:30am'])}
              </div>
            </div>
            {/* Receso */}
      ...
      >
        ...
        <div ref={page1BodyRef} className="absolute overflow-hidden" style={{ top: `${marginTopPx + HEADER_H}px`, left: `${marginLeftPx}px`, right: `${marginRightPx}px`, bottom: `${marginBottomPx + FOOTER_H}px` }}>
          <div style={{ width: '100%', fontSize: `${BASE_BODY * page1Settings.bodyFontScale}px`, lineHeight: 1.4, padding: '8px' }}>
             {/* ... (Custom Service Rendering Logic) ... */}
             {renderCustomSegments(serviceData.segments)}
          </div>
        </div>
        {/* Footer */}
        <div className="print-footer" style={{ position: 'absolute', bottom: '0.4in', left: '0.5in', width: '7.5in', height: '24px', zIndex: 9999, backgroundColor: '#1F8A70', background: 'linear-gradient(90deg, #1F8A70 0%, #4DC15F 50%, #D9DF32 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '12px', border: '1px solid #1F8A70' }}>
  ...
  // Helper to render segments for Standard (duplicated logic extracted)
  const renderStandardSegments = (segments) => {
    ...
    return segments.filter(s => s?.type !== 'break').map((seg, idx) => (
      <div key={idx} style={{ marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid #f3f4f6' }}>
        {/* ... (Original Standard Segment JSX) ... */}
        <div style={{ marginBottom: '3px', display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
          {/* ... Title, Sparkles, Duration ... */}
          <span style={{ fontSize: `${BASE_TITLE * 0.92 * page1Settings.titleFontScale}px`, fontWeight: '600', textTransform: 'uppercase', color: '#1a1a1a' }}>{seg.title || 'Sin título'}</span>
  ...
  // Helper to render segments for Custom (duplicated logic extracted)
  const renderCustomSegments = (segments) => {
    if (!Array.isArray(segments)) return null;
    return segments.map((seg, idx) => (
      <div key={idx} style={{ marginBottom: '8px', paddingBottom: '6px', borderBottom: idx < segments.length - 1 ? '1px solid #e5e7eb' : 'none' }}>
         {/* ... (Original Custom Segment JSX) ... */}
  ...
  function renderPage1() {
    return isCustomService ? renderCustomPreview() : renderStandardPreview();
  }
