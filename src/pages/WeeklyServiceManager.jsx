...
export default function WeeklyServiceManager() {
  ...
  const handleDownloadPDF = async () => {
    ...
    container.style.width = '900px'; // Increased width for better rendering
    container.style.background = 'white';
    container.style.padding = '32px 40px 60px 40px'; // Reduced padding for more space
    ...
    try {
      ...
      const margin = 5; // Small margin to prevent edge cutoff
      ...
      pdf.addImage(servicesCanvas.toDataURL('image/png'), 'PNG', margin, position + margin, imgWidth - (margin * 2), imgHeight);
      ...
      if (announcementsEl && announcementsParent) {
        ...
        pdf.addImage(announcementsCanvas.toDataURL('image/png'), 'PNG', margin, position + margin, imgWidth - (margin * 2), imgHeight);
        heightLeft -= pageHeight;
              ...
              return (
                <div key={idx} className="print-segment">
                  ...
                  {segment.data?.ministry_leader && (
                    <div className="print-segment-detail" style={{ marginTop: '6px', paddingTop: '6px', borderTop: '2px solid #e5e7eb' }}>
              ...
              return (
                <div key={idx} className="print-segment">
                  ...
                  {segment.data?.translator && (
                    <div className="print-segment-detail" style={{ background: '#eff6ff', borderLeft: '3px solid #3b82f6', color: '#1e40af' }}>
                      <strong>🌐 Trad:</strong> <span className="print-name" style={{ color: '#1e40af' }}>{segment.data.translator}</span>
                    </div>
                  )}
