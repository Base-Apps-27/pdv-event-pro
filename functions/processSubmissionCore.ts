// UNIFIED SUBMISSION PROCESSOR (Events + Services)
// Centralizes verse parsing + LLM extraction for both pipelines.
// Called by: processPendingSubmissions (events), processSegmentSubmission (services)
// VERSION: 2026-03-07 v1.0 (DECISION-007: Unified Parity Architecture)

// ═══════════════════════════════════════════════════════════════════════════
// BIBLE PARSING LOGIC (synced from parseScriptureShared, 2026-03-02)
// VERSION_HASH: v1.0-2026-03-02 (must match inline copies and parseScriptureShared)
// ═══════════════════════════════════════════════════════════════════════════
...
async function processSubmissionCore(base44, inputData) {
  // inputData shape: { content, content_is_slides_only, hasAuth? }
  // Returns: { parsed_verse_data, scriptureReferences, success, error? }
  
  let parsedData = { type: 'empty', sections: [] };
  let scriptureReferences = '';
  const isSlidesOnly = !!inputData.content_is_slides_only;

  if (!isSlidesOnly && inputData.content) {
    // STEP 1: Regex verse parsing (bilingual)
    console.log("[CORE] Parsing scripture via regex...");
    parsedData = parseScriptureReferences(inputData.content);
    ...
    // Only runs if authenticated (has auth context)
    if (inputData.content.length > 100 && inputData.hasAuth) {
      try {
        console.log("[CORE] Starting bilingual LLM extraction...");
        const prompt = `
...
Text to analyze:
${inputData.content.substring(0, 15000)}`;
        ...
        // Use LLM verses only as FALLBACK (regex takes precedence for bilingual quality)
        if (aiVerses.length > 0 && parsedData.sections.length === 0) {
          ...
          console.log(`[CORE] LLM provided ${aiVerses.length} verses (regex found none)`);
        } else if (aiVerses.length > 0) {
          console.log(`[CORE] LLM found ${aiVerses.length} verses but regex has ${parsedData.sections.length} — keeping regex`);
        }
        ...
        console.log(`[CORE] Detected lang=${detectedLang}, EN=${aiTakeawaysEn.length}, ES=${aiTakeawaysEs.length} takeaways`);
      } catch (llmErr) {
        console.error(`[CORE] LLM extraction failed: ${llmErr.message}`);
        // Silent fail — fallback to regex-only result
      }
    }
  } else if (isSlidesOnly) {
    console.log("[CORE] Slides-only mode — skipping verse parsing");
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { processSubmissionCore };
}