/**
 * parseScriptureShared — CTO-1 (P2 Audit, 2026-03-02)
 * 
 * CANONICAL backend verse parsing function. Single source of truth for
 * BIBLE_BOOKS dictionary and parseScriptureReferences regex logic.
 * 
 * Called by:
 *   - submitWeeklyServiceContent (via base44.functions.invoke)
 *   - processNewSubmissionVersion (via base44.functions.invoke)
 *   - processPendingSubmissions (via base44.functions.invoke)
 * 
 * Frontend VerseParserDialog keeps its own copy (cannot call backend
 * synchronously during typing). Frontend copy must stay in sync manually.
 * 
 * VERSION_HASH: v1.0-2026-03-02
 * If you change parsing logic here, update the VERSION_HASH and
 * also update VerseParserDialog's VERSION_HASH comment.
 * 
 * Decision: "Verse Parsing Logic Anti-Drift Protocol" (2026-02-05)
 * Decision: "CTO-1 Shared Verse Parsing" (2026-03-02)
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Auth: any authenticated user can parse verses (used by backend functions)
    ...
    if (!text || typeof text !== 'string') {
      return Response.json({ type: 'empty', sections: [] });
    }

    const result = parseScriptureReferences(text);
